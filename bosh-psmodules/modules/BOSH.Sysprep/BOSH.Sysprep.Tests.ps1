...
Describe "Set-ProtectYourPC" {
    ...
    Context "when provided a valid XML answer file containing an 'oobeSystem' block which contains a 'Microsoft-Windows-Shell-Setup' block which contains a 'ProtectYourPC' block but without specifying a ProtectYourPC value" {
        BeforeEach {
...
'@ | Out-File $GoodAnswerFilePath
    ...
    Context "when provided a valid XML answer file containing an 'oobeSystem' block which contains a 'Microsoft-Windows-Shell-Setup' block which contains a 'ProtectYourPC' block, while specifying a ProtectYourPC value" {
        BeforeEach {
            $GoodAnswerDirectory = (New-TempDir)
            $GoodAnswerFilePath = Join-Path $GoodAnswerDirectory "validanswer.xml"
